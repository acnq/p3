...
-- use pattern matching to refine the types 
def DBType.beq (t : DBType) (x y : t.asType) : Bool := 
  match t with 
  | .int => x == y
  | .string => x == y 
...
instance : BEq DBType where
  beq
    ...
    | .bool, .bool => true
    | _, _ => false
-- former comparison drawn from the types described by the codes
-- latter allows comparison of codes themselves 
...
-- that a column with a particular name and type exists in a schema.
-- 2. serves as *data* 
-- that can be used to find the value associated with the column in a row.

-- Subschemas
-- *project* a table or row into a smaller schema
-- this features requires subschema to evident some schema belongs to another
...
#eval tallInDenmark.evaluate ("Valley Bakke", "Denmark", 31, 2023)
...
def Table.cartesianProduct_ (table1 : Table s1) (table2 : Table s2) : Table (s1 ++ s2) := Id.run do 
...
-- forgetting qualifier "mountain.locaiton"
-- open Query in
-- def example2' :=
